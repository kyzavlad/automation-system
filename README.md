...
3. Add account details in `accounts-config.json` (10 accounts per platform).
...
- Generate cookies for each account using a browser extension (e.g. *Get cookies.txt*) and place them in files like `cookies_tiktok_1.txt`. The generic `cookies.txt` file is copied into service1 for downloads that require authentication.
